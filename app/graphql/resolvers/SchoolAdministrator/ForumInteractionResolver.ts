...
import { NewForumInteraction } from '../../inputs/SchoolAdministrator/NewForumInteraction';
...
} from '../../models/SchoolAdministrator/ForumInteraction';
...
export class ForumInteractionResolver {
    ...
    @FieldResolver((_type) => School, { nullable: true })
    async forum(@Root() data: ForumInteraction) {
        let id = data.forumId;
        if (id !== null && id !== undefined) {
            const result = await this.repositoryForum.findOneBy(id);
            return result;
        }
        return null;
    }
