...
export class AcademicAsignatureResolver {
  ...
  private repository = getMongoRepository(AcademicAsignature);

  @InjectRepository(User)
  private repositoryUser = getMongoRepository(User);

  @InjectRepository(AcademicArea)
  private repositoryAcademicArea = getMongoRepository(AcademicArea);
  ...
  async updateAcademicAsignature(
    ...
    @Ctx() context: IContext
  ): Promise<AcademicAsignature | undefined> {
  ...
  async academicArea(@Root() data: AcademicAsignature) {
    ...
    if (id !== null && id !== undefined) {
      const result = await this.repositoryAcademicArea.findOne(id);
      return result;
    }
    return null;
  }
